...
from crownline_maturity_time_engine import _search_maturity_structural_tt_with_deadline
...
from crownline_tt_time_engine import TTIterationSearchRecord, TTIterativeDeepeningStats, _TTCounter


@dataclass(frozen=True)
class MaturityProductDecisionStats:
...
@dataclass
class RepeatAwareMaturityStructuralTTOpponent:
    """Measured Crownline product composition candidate.
    ...
    Independent Stage-3 additions:
      * p200 exact-history root penalty for repeated afterstates;
      * promotion maturity w10 as the only strategic evaluator extension.

    Crownline v1.1 rules, legal moves, alpha-beta semantics, and deterministic
    tie-breaking are unchanged. This class is experimental and is not the browser
    opponent unless a later product gate explicitly promotes it.
    """

    name: str = "stage2-tt-plus-repeat-p200-plus-maturity-w10"
    ...
    def _maybe_reset_for_game(self, state: CrownlineSet) -> None:
        ...
        if (
            ...
            or (self._last_seen_ply is not None and game.ply <= self._last_seen_ply)
        ):
    ...
    def _choose(
    ...
    ) -> tuple[str, Optional[Line], MaturityProductDecisionStats]:
        if state.set_over:
            ...
            try:
                ...
                iterations.append(
                    TTIterationSearchRecord(
                        depth=depth,
                        completed=False,
                        elapsed_ms=max(0.0, (self.clock_ns() - iteration_started_ns) / 1_000_000.0),
        ...
        stats = MaturityProductDecisionStats(
            search=search_stats,
    ...
    def choose_with_stats(
        ...
        self._produced_afterstates[stats.selected_afterstate] = (
            self._produced_afterstates.get(stats.selected_afterstate, 0) + 1
        )
        self.decisions += 1
