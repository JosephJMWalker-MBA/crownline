...
@dataclass
class OrderedSearchStats:
    """Per-decision measurements for evaluator-guided move ordering."""
...
def _ordered_children(
    ...
    *,
    maximizing: bool,
    stats: OrderedSearchStats,
) -> tuple[tuple[CrownlineSet, str, str], ...]:
    """Order children by the unchanged static evaluator, best-looking first.

    The ordering score is used only to improve alpha-beta cutoff opportunity. It
    does not replace the minimax value and therefore cannot change the intended
    policy when the search implementation is correct.
    """

    ranked = []
    for move, meld_line in _actions(state):
        child = state.apply_move(move, meld_line=meld_line)
        estimate = _evaluate(child, participant)
    ...
    if maximizing:
        ...
        ranked.sort(key=lambda item: (item[0], item[1], item[2]))
    return tuple((item[3], item[1], item[2]) for item in ranked)


def _search_static_ordered(
    state: CrownlineSet,
    participant: Participant,
    depth: int,
    alpha: float,
...
def choose_computer_action_static_ordered(
    state: CrownlineSet,
    participant: Participant = "B",
    *,
    depth: int = 2,
) -> tuple[str, Optional[Line], OrderedSearchStats]:
    """Choose Baseline A's policy with evaluator-guided internal move ordering."""

    if depth < 1 or depth > 4:
        raise ValueError("Ordered search depth must be between 1 and 4")
    if state.set_over:
        raise ValueError("Set is already over")
    game = state.current_game
    if game.game_over:
    ...
    stats = OrderedSearchStats()
    ranked = []
    # Root actions deliberately remain in the baseline order. Each root child is
    # searched with a full window, and the existing lexicographic tie-break is
    # preserved exactly.
    for move, meld_line in actions:
        child = state.apply_move(move, meld_line=meld_line)
        value = _search_static_ordered(
            child,
            participant,
            max(0, depth - 1),
            -inf,
...
@dataclass
class StaticOrderedBaselineEngine:
    """Baseline A + static-evaluator internal move ordering."""

    name: str
    depth: int = 2
    total_expanded_nodes: int = 0
    total_cutoff_nodes: int = 0
    total_ordering_evaluations: int = 0

    def __post_init__(self) -> None:
        if self.depth < 1 or self.depth > 4:
            raise ValueError("StaticOrderedBaselineEngine depth must be between 1 and 4")

    def choose(self, state: CrownlineSet, participant: Participant) -> EngineDecision:
        root_actions = len(_actions(state))
        started = perf_counter_ns()
        notation, meld_line, stats = choose_computer_action_static_ordered(
            state,
            participant=participant,
            depth=self.depth,
        )
        ...
        self.total_expanded_nodes += stats.expanded_nodes
        self.total_cutoff_nodes += stats.cutoff_nodes
        self.total_ordering_evaluations += stats.ordering_evaluations
