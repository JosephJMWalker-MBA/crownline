...
from crownline_promotion_utility_experiment import choose_promotion_proximity_action
...
def _root_policy_changes(fixtures, *, depth: int, weight: float) -> dict:
    ...
    for fixture_id, fixture in fixtures:
        ...
        candidate = choose_promotion_proximity_action(
            state,
            participant,
            depth=depth,
...
def _cycle_policy(depth: int, weight: float) -> dict:
    ...
    for index, (state, cycle_move) in enumerate(_cycle_states(), start=1):
        participant = state.participant_for_color(state.current_game.turn)
        selected = choose_promotion_proximity_action(
            state,
            participant,
            depth=depth,
            promotion_weight=weight,
...
def run(weights: tuple[float, ...], *, root_depth: int) -> dict:
    ...
    return {
        ...
        "experiment": "stage3-promotion-proximity-root-diagnostic",
        "rules_mode": "candidate",
        "root_depth": root_depth,
        "weights": list(weights),
        "hypothesis": (
            "A nonlinear nonterminal value for ordinary pieces approaching promotion may correct "
            "a Crownline-specific strategic omission without assigning a constant King bonus. "
            "The first gate is policy disruption on both the early control suite and the "
            "artifact-derived King hard cases, not full-trajectory win rate."
        ),
        "points": points,
        "source_fingerprints": {
            ...
            "diagnostic_sha256": _source_fingerprint(
                "experiments/benchmark_promotion_proximity_diagnostic.py"
            ),
        },
    }
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Sweep one promotion-proximity evaluator weight across early and King-rich root guardrails."
    )
    parser.add_argument(
        "--weights",
        nargs="+",
        type=float,
        default=(0.0, 25.0, 50.0, 100.0, 200.0, 400.0),
    )
...
def main() -> None:
    ...
    print("Crownline Stage 3 promotion-proximity root diagnostic")
    ...
    for point in report["points"]:
        ...
        print(
            f"w={point['promotion_weight']:g}: early changes "
